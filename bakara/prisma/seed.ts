...
import { CHECK_ITEMS, Checkpoint, itemApplies, dueMinutes, CheckItem } from "../src/lib/checks";
import { addDays, nowIL, parseLocalDateTime, fromMinutes } from "../src/lib/time";
import { buildShiftSummary } from "../src/lib/summary";
...
async function main() {
  ...
  async function runShift(opts: {
    ...
    for (const c of present) {
      ...
      for (const item of items) {
        ...
        rows.push({ date, childId: c.id, itemKey: item.key, status, independence, shiftId: shift.id, updatedById: managerId, createdAt: at, updatedAt: at, item, child: c });
      }
    }
    for (const r of rows) {
      const { item, child, ...data } = r;
      await db.checkEntry.create({ data });
      if (r.status === "NOT_DONE" || r.status === "NEEDS_CARE") {
        await createException({ date, item, childId: child.id, shiftId: shift.id, openedById: managerId, at: r.createdAt, closeIt: opts.close && (dayIndex < 12 || rnd() < 0.7) });
      }
    }
    if (opts.close) {
  ...
  console.log("יוצר היסטוריה של 14 ימים...");
  for (let d = 14; d >= 1; d--) {
  ...
  console.log("יוצר את היום הנוכחי...");
  ...
  console.log(`נוצרו ${counts[0]} ילדים, ${counts[1]} משמרות, ${counts[2]} בדיקות, ${counts[3]} חריגות פעילות.`);
  console.log("משתמשים: admin / director / duty1 / duty2 / duty3, סיסמה: demo1234");
}
